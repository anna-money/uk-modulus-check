...
class UKModulusChecker:
    __slots__ = (
        "__sort_code_substitution_table",
        "__weight_table",
    )
    ...
    def __init__(
        ...
        self.__weight_table = weight_table
        self.__sort_code_substitution_table = sort_code_substitution_table

    @classmethod
    def __calc_mod_dblal(
        cls,
    ...
    @classmethod
    def __calc_mod10(cls, weighted_sort_code_parts: list[int], weighted_account_number_parts: list[int]) -> int:
        weighted_sum = sum(weighted_sort_code_parts + weighted_account_number_parts)
        return weighted_sum % 10

    @classmethod
    def __calc_mod11(cls, weighted_sort_code_parts: list[int], weighted_account_number_parts: list[int]) -> int:
        weighted_sum = sum(weighted_sort_code_parts + weighted_account_number_parts)
    ...
    def __calc_mod(
        ...
        if rule.exception == 5:
            new_sort_code = self.__sort_code_substitution_table.try_get_substitution(sort_code)
        ...
        if rule.exception == 10:
            ...
            if account_number_parts[0] in (4, 5, 6, 7, 8) and account_number_parts[6] == account_number_parts[7]:
                return True
        elif rule.exception == 7 and account_number_parts[6] == 9:
            weighted_sort_code_parts = [0, 0, 0, 0, 0, 0]
            weighted_account_number_parts[0] = 0
            weighted_account_number_parts[1] = 0
        ...
        match rule.mod_mode:
            case ModMode.DblAl:
                if rule.exception == 3 and account_number_parts[2] in (6, 9):
                    return True
                remainder = self.__calc_mod_dblal(weighted_sort_code_parts, weighted_account_number_parts, rule)
            case ModMode.Mod10:
                remainder = self.__calc_mod10(weighted_sort_code_parts, weighted_account_number_parts)
            case ModMode.Mod11:
                remainder = self.__calc_mod11(weighted_sort_code_parts, weighted_account_number_parts)
    ...
    def validate(self, sort_code: int, account_number: int) -> ValidationResult:
        sort_code_rules = self.__weight_table.try_get_rules(sort_code)
        ...
        for rule in sort_code_rules:
            if rule.exception == 9:
                mod_pass = self.__calc_mod(309634, account_number, rule)
                if mod_pass:
                    return ValidationResult(result=True, known_sort_code=True, substitute_sort_code=309634)
            else:
                mod_pass = self.__calc_mod(sort_code, account_number, rule)
            if exception_10_11 or exception_12_13 or exception_2_9:
                ...
                if mod_pass:
                    return ValidationResult(result=True, known_sort_code=True, substitute_sort_code=None)
                mod_pass = self.__calc_mod(sort_code, account_number, rule, exception_14=True)
